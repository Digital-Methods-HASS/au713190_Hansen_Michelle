# so the homework is at the bottum i simply couldn't get it to work in a new project, so i just figured i would continue in here.
...
# there are some extra spaces, that we do not see in the pdf, but we do see that in R. we also see in R that it consider the index and layout.
...
# Homework (Game of thrones)

# i tried to make a seperate project but it couldn't find the got path file so i continued in the pratice, and it worked.
...
# here i trim the text to get rid of \n like in the tortorial 
...
# here i am just re counting to see if it worked and it did yay.
got_swc <- got_stop %>%
  count(word) %>%
  arrange(-n)
got_swc

# honestly... i don't know what the warning means, and i also got it in the totorial so i hope it's not important. 
What if we want to get rid of all the numbers (non-text) in `ipcc_stop`?
```{r skip-numbers}
# This code will filter out numbers by asking:
# If you convert to as.numeric, is it NA (meaning those words)?
...
#making the word cloud prettier, it doesn't box in color but it somehow works so i'll take it 
...
ggsave(plot = got_nrc_gg,
       ...
       height = 8,
       width = 5)
# So i think that's that, thank the gods for the totorial, otherwise i would have been stuck here until christmas. I found that there was a more positive words than negative, which at first surpried me, but when i think about it make sense, that it's the other semtiments like, fear and anger that show, and not negativaty, that wouldn't sell a good show.
